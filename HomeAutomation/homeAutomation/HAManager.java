		...
		for (Component c : components){
			if (c.getName()==name && c.getStatus() != stat){ //if current status is not stat
				...
				for (Automation a : automations){ //loop through automations
					if (a.getTriggerComp() == c && a.getTriggerStatus() == stat){ // find triggered automations
						chainedAutos = a.triggerAutomation(automations); // trigger that automation. first triggered automation now contains all names of all chained automations and returns it to the HAManager instance
